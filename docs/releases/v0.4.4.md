...
Caddy is the first Compose reference provider. It is application-scoped in the provider registry, owns only its own routing state, creates a stable exposure network and attaches only explicitly exposed services through BaseHarbor's generated override.
