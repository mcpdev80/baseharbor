...
12. The release workflow consumes the successful immutable pre-release approval instead of rerunning the same source/runtime/demo matrix. It performs only release-only checks not already covered, publishes the matching runtime image, GitHub Release, archives and provenance.
