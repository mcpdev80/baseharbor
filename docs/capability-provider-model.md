...
| Capability | Portable interface / intent | BaseHarbor default | Status through v0.4.10 | Replacement paths / alternatives | Architecture note |
| --- | --- | --- | --- | --- | --- |
...
| Metrics | `metrics/v1` / OpenMetrics-compatible HTTP exposition | Prometheus 3.14.0 Compose reference provider; shared default, named shared boundaries and application-scoped placement | v0.4.10: application sources plus generic policy-authorized metrics advertised by managed providers; automatic registration and real scrape verification | VictoriaMetrics, Mimir and compatible backends | Applications declare only their metrics source. Providers may advertise their own safe observability signals through the Provider Integration Contract; Prometheus does not contain product-specific scrape branches |
...
v0.4.10 remains Compose-only at runtime. The v0.4 line now includes the shared capability/provider/resource/binding core, protected provider placement/ownership, the Provider Integration Contract v1, deterministic repository inspection, managed traffic through `exposure.http/v1`, the shared `secure-binding/v1` security boundary, `object-storage.s3/v1`, provider-neutral `telemetry.otlp/v1`, `metrics/v1`, `logs/v1` and `traces/v1`. Provider descriptors can advertise observability signals generically, while collection policy and placement decide whether those signals are connected. It also keeps explicit directional cross-application connectivity as a separate deny-by-default platform policy.
