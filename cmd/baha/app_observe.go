...
func appStatusCommand(store application.Store) *cli.Command {
	return &cli.Command{
		...
		Long:    "Reports materialized runtime state, running services and protocol-level readiness. Running containers are not considered ready unless each enabled service passes its authenticated verification and every declared required secret is present and readable.",
				...
				if platformErr != nil {
					...
					if err != nil {
						...
						if len(application.RequiredSecretNames(m)) > 0 {
							statuses, statusErr := inspectRequiredSecrets(checkCtx, compose, platformFiles, m, files)
							printRequiredSecretStatus(out, statuses)
							if statusErr != nil || requireReadySecrets(statuses) != nil {
...
func appDoctorCommand(store application.Store) *cli.Command {
	return &cli.Command{
		...
		Long:    "Checks desired state, secure local runtime files, Compose configuration, service state, authenticated protocol readiness, managed OpenBao scope health and required-secret readiness without mutating the application.",
		Run: func(ctx context.Context, args []string, out, errOut io.Writer) error {
			if len(args) != 1 {
