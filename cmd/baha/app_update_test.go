...
func TestInspectGitApplicationUpdateDetectsFastForwardAndDirtyTree(t *testing.T) {
	...
	if state.Branch != "main" || state.Upstream != "origin/main" || state.Remote != "origin" { t.Fatalf("unexpected Git tracking state: %#v", state) }
	if state.Relation != "update-available" || state.Dirty || state.Current == state.Target { t.Fatalf("expected clean fast-forward update: %#v", state) }
	...
	if err != nil { t.Fatal(err) }
	if !state.Dirty || state.Relation != "update-available" { t.Fatalf("expected dirty tree with update available: %#v", state) }
}

func TestInspectGitApplicationUpdateRejectsDetachedHead(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil { t.Skip("git is not available") }
	...
	if err == nil || !strings.Contains(err.Error(), "detached HEAD") { t.Fatalf("expected detached HEAD rejection, got %v", err) }
}
...
func TestParseAppUpdateOptionsEnforcesRecoveryChoiceRules(t *testing.T) {
	...
	if err != nil || opts.BackupPasswordFile != "/tmp/password" || opts.NoBackup || opts.Check { t.Fatalf("unexpected parsed options: %#v err=%v", opts, err) }
	if _, err := parseAppUpdateOptions([]string{"--backup-password-file", "/tmp/password", "--no-backup"}); err == nil { t.Fatal("expected mutually exclusive recovery policy error") }
	if _, err := parseAppUpdateOptions([]string{"--check", "--no-backup"}); err == nil { t.Fatal("expected read-only check to reject backup options") }
}

func TestApplicationUpdateDurableStateDetection(t *testing.T) {
	if applicationUpdateHasDurableState(application.Manifest{}) { t.Fatal("empty manifest should not require a recovery point") }
	if !applicationUpdateHasDurableState(application.Manifest{Services: application.Services{Postgres: true}}) { t.Fatal("PostgreSQL should be durable") }
	if !applicationUpdateHasDurableState(application.Manifest{Services: application.Services{Secrets: true}}) { t.Fatal("managed secrets should be durable") }
}
...
func TestApplicationUpdateCommandIsDiscoverable(t *testing.T) {
	...
	for _, child := range root.Children {
		if child.Name != "app" { continue }
		appFound = true
		for _, appChild := range child.Children {
			if appChild.Name == "update" {
				updateFound = true
				if appChild.Usage != "baha app update [--check] [--backup-password-file FILE | --no-backup]" { t.Fatalf("unexpected update usage: %s", appChild.Usage) }
			}
		}
	}
	if !appFound || !updateFound { t.Fatalf("application update command not discoverable: app=%v update=%v", appFound, updateFound) }
}
