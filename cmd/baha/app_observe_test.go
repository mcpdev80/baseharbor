...
func TestApplicationComponentsStopped(t *testing.T) {
	tests := []struct {
		...
		{name: "managed exposure running", workloadFound: true, want: false},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := applicationComponentsStopped(test.managed, test.workload, test.workloadFound, test.brokerRunning, false); got != test.want {
