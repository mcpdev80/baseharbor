...
func TestAdoptionSummaryMinimalWorkload(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Environment   dev",
		"Workload",
		"Compose       compose.yaml (repository-owned, read-only)",
		"Services      api",
	} {
		if !strings.Contains(text, want) {
			t.Fatalf("summary missing %q:\n%s", want, text)
...
func TestAdoptionSummaryFullyPopulated(t *testing.T) {
	...
	m = application.WithLogsCollection(m, "application")
	m = application.WithRuntimePermission(m, "object-storage.s3/v1", []string{"api"}, "runtime.create")
	policies := []guidedSecretPolicy{{Name: "API_TOKEN", Required: true, Provision: "prompt"}}
	var out bytes.Buffer
	printAdoptionSummary(&out, m, appProjectDetection{Postgres: true, Redis: true, ObjectStorage: true}, policies)
	text := out.String()
	for _, want := range []string{
		"SQL Database  detected and confirmed; default provider PostgreSQL",
		"Cache         detected and confirmed; default provider Valkey/Redis-compatible",
		"Object Storage detected and confirmed; S3-compatible",
		"Metrics       /metrics",
		"OTLP          traces",
		"Logs          application",
		"API_TOKEN: required for startup; ask securely during first apply",
