...
func TestResolveServiceBindingDefaultPostgres(t *testing.T) {
	...
	for name, value := range map[string]string{
		...
		if err := os.WriteFile(filepath.Join(dir, name), []byte(value), 0o600); err != nil { t.Fatal(err) }
	}
...
func TestResolveServiceBindingNamedValkey(t *testing.T) {
	...
	if err := os.MkdirAll(dir, 0o700); err != nil { t.Fatal(err) }
	for name, value := range map[string]string{
		"host": "127.0.0.1", "port": "16379", "password": "secret", "uri": "redis://:secret@127.0.0.1:16379/0",
	} {
		if err := os.WriteFile(filepath.Join(dir, name), []byte(value), 0o600); err != nil { t.Fatal(err) }
	}
...
func TestResolveServiceBindingMissingFailsClosed(t *testing.T) {
	...
	if err == nil { t.Fatal("expected missing materialized binding to fail") }
}
