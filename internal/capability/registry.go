...
func (r *Registry) Register(instance ProviderInstance) error {
	if r == nil { return errors.New("provider registry is nil") }
	if r.Version == 0 { r.Version = RegistryVersion }
	if err := validateProviderInstance(instance); err != nil { return err }
	for _, existing := range r.Instances {
		if existing.ID == instance.ID {
			if sameProviderInstance(existing, instance) { return nil }
...
func (r Registry) Resolve(provider ProviderKind, scope ProviderScope, application, externalID string) (ProviderInstance, error) {
	...
	for _, instance := range r.Instances {
		if instance.Provider.Kind != provider || instance.Scope != scope { continue }
		switch scope {
		case ScopeShared:
			matches = append(matches, instance)
		case ScopeApplication:
			if instance.OwnerApplication == application { matches = append(matches, instance) }
		case ScopeExternal:
			if externalID != "" && instance.ID == externalID { matches = append(matches, instance) }
		}
	}
	if len(matches) == 0 { return ProviderInstance{}, fmt.Errorf("provider %q with scope %q not found", provider, scope) }
	if len(matches) != 1 { return ProviderInstance{}, fmt.Errorf("provider %q with scope %q is ambiguous", provider, scope) }
...
func (r *Registry) Bind(resource Resource, providerInstanceID string) error {
	...
	if !ok { return fmt.Errorf("provider instance %q not found", providerInstanceID) }
	...
	for _, binding := range r.Bindings {
		if sameLogicalResource(binding.Resource, resource) {
			if binding.ProviderInstanceID == providerInstanceID { return nil }
...
func (r Registry) ApplicationLifecycle(application string, operation LifecycleOperation) ([]LifecycleAction, error) {
	application = strings.TrimSpace(application)
	if application == "" { return nil, errors.New("application is required") }
	switch operation {
	...
	for _, binding := range r.Bindings {
		if binding.Resource.Application != application { continue }
		if _, exists := seen[binding.ProviderInstanceID]; exists { continue }
		instance, ok := r.instance(binding.ProviderInstanceID)
		if !ok { return nil, fmt.Errorf("binding references missing provider instance %q", binding.ProviderInstanceID) }
...
func (r Registry) Validate() error {
	if r.Version != RegistryVersion { return fmt.Errorf("unsupported provider registry version %d", r.Version) }
	seenIDs := map[string]struct{}{}
	shared := map[ProviderKind]string{}
	for _, instance := range r.Instances {
		if err := validateProviderInstance(instance); err != nil { return err }
		if _, exists := seenIDs[instance.ID]; exists { return fmt.Errorf("duplicate provider instance %q", instance.ID) }
...
func (s RegistryStore) Load() (Registry, error) {
	...
	if errors.Is(err, os.ErrNotExist) { return NewRegistry(), nil }
	if err != nil { return Registry{}, fmt.Errorf("read provider registry: %w", err) }
...
func (s RegistryStore) Save(registry Registry) error {
	if strings.TrimSpace(s.Path) == "" { return errors.New("provider registry path is required") }
	if err := registry.Validate(); err != nil { return err }
	...
	sort.Slice(registry.Bindings, func(i, j int) bool {
		...
		if a.Application != b.Application { return a.Application < b.Application }
		if a.Kind != b.Kind { return a.Kind < b.Kind }
		return a.Name < b.Name
	})
	data, err := json.MarshalIndent(registry, "", "  ")
	if err != nil { return fmt.Errorf("encode provider registry: %w", err) }
	data = append(data, '\n')
	if err := os.MkdirAll(filepath.Dir(s.Path), 0o700); err != nil { return fmt.Errorf("create provider registry directory: %w", err) }
	tmp := s.Path + ".tmp"
	if err := os.WriteFile(tmp, data, 0o600); err != nil { return fmt.Errorf("write provider registry: %w", err) }
	if err := os.Chmod(tmp, 0o600); err != nil { _ = os.Remove(tmp); return fmt.Errorf("secure provider registry: %w", err) }
	if err := os.Rename(tmp, s.Path); err != nil { _ = os.Remove(tmp); return fmt.Errorf("replace provider registry: %w", err) }
	return nil
}

func validateProviderInstance(instance ProviderInstance) error {
	if strings.TrimSpace(instance.ID) == "" { return errors.New("provider instance id is required") }
	if instance.Provider.Kind == "" || len(instance.Provider.Capabilities) == 0 { return fmt.Errorf("provider instance %q requires a provider descriptor", instance.ID) }
	switch instance.Scope {
	case ScopeShared:
		if instance.OwnerApplication != "" { return fmt.Errorf("shared provider instance %q cannot have an application owner", instance.ID) }
		if instance.Ownership != OwnershipBaseHarbor { return fmt.Errorf("shared provider instance %q must be BaseHarbor-owned; use external scope for BYO providers", instance.ID) }
	case ScopeApplication:
		if strings.TrimSpace(instance.OwnerApplication) == "" { return fmt.Errorf("application-scoped provider instance %q requires an owner application", instance.ID) }
		if instance.Ownership != OwnershipBaseHarbor { return fmt.Errorf("application-scoped provider instance %q must be BaseHarbor-owned", instance.ID) }
	case ScopeExternal:
		if instance.Ownership != OwnershipExternal { return fmt.Errorf("external provider instance %q must have external ownership", instance.ID) }
		if strings.TrimSpace(instance.Reference) == "" { return fmt.Errorf("external provider instance %q requires a non-secret reference", instance.ID) }
		if instance.OwnerApplication != "" { return fmt.Errorf("external provider instance %q cannot be lifecycle-owned by an application", instance.ID) }
	default:
...
func (r Registry) instance(id string) (ProviderInstance, bool) {
	for _, instance := range r.Instances {
		if instance.ID == id { return instance, true }
	}
...
func sameProviderInstance(a, b ProviderInstance) bool {
	if a.ID != b.ID || a.Provider.Kind != b.Provider.Kind || a.Scope != b.Scope ||
		...
		len(a.Provider.Capabilities) != len(b.Provider.Capabilities) { return false }
	for i := range a.Provider.Capabilities {
		if a.Provider.Capabilities[i] != b.Provider.Capabilities[i] { return false }
	}
	return true
}
