...
func TestRegistryReusesOneSharedProviderAcrossApplications(t *testing.T) {
	...
	if err := registry.Register(shared); err != nil { t.Fatal(err) }
	if err := registry.Register(shared); err != nil { t.Fatalf("idempotent register: %v", err) }
	for _, app := range []string{"alpha","beta"} {
		resource, err := Resolve(app, Requirement{Kind:Secrets,Name:"default"}, OpenBao)
		if err != nil { t.Fatal(err) }
		if err := registry.Bind(resource, shared.ID); err != nil { t.Fatal(err) }
	}
	if len(registry.Instances) != 1 || len(registry.Bindings) != 2 { t.Fatalf("registry=%#v", registry) }
}

func TestRegistryRejectsDuplicateSharedProvider(t *testing.T) {
	registry := NewRegistry()
	if err := registry.Register(ProviderInstance{ID:"openbao/a",Provider:OpenBao,Scope:ScopeShared,Ownership:OwnershipBaseHarbor}); err != nil { t.Fatal(err) }
	if err := registry.Register(ProviderInstance{ID:"openbao/b",Provider:OpenBao,Scope:ScopeShared,Ownership:OwnershipBaseHarbor}); err == nil { t.Fatal("duplicate shared provider accepted") }
}
...
func TestRegistryApplicationProviderCannotCrossApplicationBoundary(t *testing.T) {
	...
	if err := registry.Bind(resource, instance.ID); err == nil { t.Fatal("cross-application binding accepted") }
}
...
func TestRegistryExternalProviderIsBindableButNeverLifecycleOwned(t *testing.T) {
	...
	if err := registry.Bind(resource, instance.ID); err != nil { t.Fatal(err) }
	actions, err := registry.ApplicationLifecycle("alpha", LifecycleDestroy)
	if err != nil { t.Fatal(err) }
	if len(actions) != 1 || actions[0].MutateProvider || !actions[0].RemoveBinding { t.Fatalf("actions=%#v", actions) }
}
...
func TestRegistryLifecycleOwnsOnlyDedicatedManagedProvider(t *testing.T) {
	...
	shared := ProviderInstance{ID:"openbao/control-plane",Provider:OpenBao,Scope:ScopeShared,Ownership:OwnershipBaseHarbor}
	for _, instance := range []ProviderInstance{dedicated,shared} { if err := registry.Register(instance); err != nil { t.Fatal(err) } }
	...
	if err := registry.Bind(sql, dedicated.ID); err != nil { t.Fatal(err) }
	if err := registry.Bind(secrets, shared.ID); err != nil { t.Fatal(err) }
	actions, err := registry.ApplicationLifecycle("alpha", LifecycleBackup)
	if err != nil { t.Fatal(err) }
	var dedicatedOwned, sharedOwned bool
	for _, action := range actions {
		if action.ProviderInstanceID == dedicated.ID { dedicatedOwned = action.MutateProvider }
		if action.ProviderInstanceID == shared.ID { sharedOwned = action.MutateProvider }
	}
	if !dedicatedOwned || sharedOwned { t.Fatalf("actions=%#v", actions) }
}
...
func TestRegistryStorePersistsOwnerOnlyValidatedState(t *testing.T) {
	...
	registry := NewRegistry()
	if err := registry.Register(ProviderInstance{ID:"openbao/control-plane",Provider:OpenBao,Scope:ScopeShared,Ownership:OwnershipBaseHarbor}); err != nil { t.Fatal(err) }
	if err := store.Save(registry); err != nil { t.Fatal(err) }
