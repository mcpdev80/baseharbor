...
The current default-branch control-plane file location is still being corrected so machine/user-scoped Docker state and manager credentials remain stable across application checkouts. That global-state fix is a blocker for the first official release and is tracked in the pre-release documentation audit.
